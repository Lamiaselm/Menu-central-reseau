...
class Domaine{
	...
	protected static $nom_table="domaines"; 
	protected static $champs = array('id', 'lib_Domaine','img','is_popular','is_active','is_new');
	public $id;
	public $lib_Domaine;
	public $img;
	public $is_popular;
	...
	public static function getDomaine_notpopular_notactive(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= "  WHERE `is_active` = 0 AND `is_popular` = 0 "; 
    return  self::trouve_par_sql($q); 
	}

	public static function getDomaine_notpopular_active(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= "  WHERE `is_active` = 1 AND `is_popular` = 0 "; 
    return  self::trouve_par_sql($q); 
	}

	public static function getDomaine_popular_notactive(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= "  WHERE `is_active` = 0 AND `is_popular` = 1 "; 
    return  self::trouve_par_sql($q); 
	}

	public static function getDomaine_popular_active(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE is_active = 1 and is_popular =1 "; 
    return  self::trouve_par_sql($q); 
	}
	public static function getDomaine_notpopular(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= "  WHERE  `is_popular` = 0 "; 
    return  self::trouve_par_sql($q); 
	}
	public static function getDomaine_popular(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE is_popular =1 "; 
    return  self::trouve_par_sql($q); 
	}
	public static function getDomaine_notactive(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= "  WHERE  `is_active` = 0 "; 
    return  self::trouve_par_sql($q); 
	}
	public static function getDomaine_active(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE is_active =1 "; 
    return  self::trouve_par_sql($q); 
	}
	public static function getDomaine_new(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE is_new =1 "; 
    return  self::trouve_par_sql($q); 
	}
	public static function getDomaine_notnew(){
	$q  =  "SELECT * FROM ".self::$nom_table;
	$q .= " WHERE is_new =0 "; 
    return  self::trouve_par_sql($q); 
	}
  public function nom_compler() {
	...
	public  function  existe(){
	 global $bd;
	 $sql  = "SELECT * FROM ".self::$nom_table." ";
    $sql .= "WHERE lib_Domaine = '".$this->lib_Domaine."' ";
    $sql .= "LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}

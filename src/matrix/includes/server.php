...
class Server{
	...
	protected static $champs = array('id', 'id_DetailDomaine','nom','acronyme', 'logo', 'editeur','image','url_server','url_editeur','longitude','latitude','pays','region','is_Djiant','adr_line_1','adr_line_2','adr_line_3','ville','code_postal','about','phone','zoom','indicatif','authorized','created_at','updated_at','created_by','updated_by','keywords');
	public $id;
    ...
    public static function exportProductDatabase($productResult) {
        $timestamp = time();
  ...
  public static function trouve_par_pays_network($id=0,$network=0) {
	...
	public static function trouve_authorized_markers_par_network($network){
	$q =  "SELECT *  FROM ".self::$nom_table;
	$q .= " WHERE id_DetailDomaine =$network";
	$q .= " and authorized =1 ";
	$q .= " order by is_Djiant ASC ";
    return  self::trouve_par_sql($q);
	}
  ...
  public function nom_compler() {
	...
	$sql .= "and url_server = '".$this->url_server."' ";
	$sql .= "and id_DetailDomaine = '".$this->id_DetailDomaine."' ";
    $sql .= "LIMIT 1";
    $result_array = self::trouve_par_sql($sql);
		return !empty($result_array) ? array_shift($result_array) : false;
	}
